...
if (typeof YAHOO.lacuna.MapPlanet == 'undefined' || !YAHOO.lacuna.MapPlanet) {
  (function () {
    ...
    MapPlanet.prototype = {
      _buildDetailsPanel: _.once(function () {
        ...
        this.buildingDetails.hideEvent.subscribe(
          function () {
      ...
      _buildBuilderPanel: _.once(function () {
        ...
        this.buildingBuilder.hideEvent.subscribe(
          function () {
        ...
        this.buildingBuilder.viewBuildings = function (mainTag, subTag) {
          ...
          if (subTag) {
            this.filterTrail.innerHTML =
              ' &gt; <a class="buildMenuLink" href="#' +
              mainTag +
              '">' +
              displayTag +
              '</a> &gt; <span class="buildMenuLink">' +
              subTag +
              '</span>';
          } else {
            this.filterTrail.innerHTML =
              ' &gt; <span class="buildMenuLink">' + displayTag + '</span>';
          }
        ...
        this.buildingBuilder.updateDisplay = function () {
          ...
          for (var i = 0; i < names.length; i++) {
            ...
            nLi.innerHTML = [
              ...
              '    <div class="yui-u first" style="width:200px;background:transparent url(',
              Lacuna.MapPlanet.surfaceUrl,
              ') no-repeat center;text-align:center">',
              '        <img src="',
              Lib.AssetUrl,
              'planet_side/',
              bd.image,
              '.png" style="width:200px;height:200px;cursor:pointer" class="buildingImage" />',
              '    </div>',
              ...
              Game.GetBuildingDesc(bd.url),
              '</div>',
              '        <table class="buildingStats" cellpadding="0" cellspacing="0">',
              '            <tr><td></td>',
              '                <th><img src="',
              Lib.AssetUrl,
              'ui/s/food.png" title="Food" class="smallFood" /></th>',
              '                <th><img src="',
              Lib.AssetUrl,
              'ui/s/ore.png" title="Ore" class="smallOre"  /></th>',
              '                <th><img src="',
              Lib.AssetUrl,
              'ui/s/water.png" title="Water" class="smallWater" /></th>',
              '                <th><img src="',
              Lib.AssetUrl,
              'ui/s/energy.png" title="Energy" class="smallEnergy" /></th>',
              '                <th><img src="',
              Lib.AssetUrl,
              'ui/s/waste.png" title="Waste" class="smallWaste" /></th>',
              '                <th><img src="',
              Lib.AssetUrl,
              'ui/s/happiness.png" title="Happiness" class="smallHappy" /></th>',
              '            </tr>',
              '            <tr><th>Cost:</th>',
              '                <td class=',
              costs.food > planet.food_stored ? 'low-resource' : '',
              '>',
              costs.food,
              '</td>',
              '                <td class=',
              costs.ore > planet.ore_stored ? 'low-resource' : '',
              '>',
              costs.ore,
              '</td>',
              '                <td class=',
              costs.water > planet.water_stored ? 'low-resource' : '',
              '>',
              costs.water,
              '</td>',
              '                <td class=',
              costs.energy > planet.energy_stored ? 'low-resource' : '',
              '>',
              costs.energy,
              '</td>',
              '                <td>',
              ...
              '            <tr><th>Prod:</th>',
              '                <td class=',
              -1 * prod.food_hour > planet.food_hour ? 'low-resource' : '',
              '>',
              prod.food_hour,
              '/hr</td>',
              '                <td class=',
              -1 * prod.ore_hour > planet.ore_hour ? 'low-resource' : '',
              '>',
              prod.ore_hour,
              '/hr</td>',
              '                <td class=',
              -1 * prod.water_hour > planet.water_hour ? 'low-resource' : '',
              '>',
              prod.water_hour,
              '/hr</td>',
              '                <td class=',
              -1 * prod.energy_hour > planet.energy_hour ? 'low-resource' : '',
              '>',
              prod.energy_hour,
              '/hr</td>',
              '                <td>',
              ...
              isNotBuildable
                ? ''
                : '            <button style="width: 100px; height: 30px; font-size: 12pt">Build</button>',
              '            <img class="smallTime" style="margin-left: ',
              isNotBuildable ? '120px' : '20px',
              '; vertical-align: middle" src="',
              Lib.AssetUrl,
              'ui/s/time.png" />',
              Lib.formatTime(costs.time),
              ' sec',
              '        </div>',
              '    </div>',
              '</div>',
      ...
      Mapper: function (oArgs) {
        ...
        Dom.setStyle(
          'buildingDetailsImgBkgd',
          'background',
          ['transparent url(', this.surfaceUrl, ') no-repeat left top'].join('')
        );
        ...
        if (!this._gridCreated) {
          ...
          Event.delegate(
            ...
            function (e, matchedEl, container) {
              if (!this._map.controller.isDragging()) {
                ...
                if (tile && tile.data) {
                  ...
                  //if(planet.size*1 > planet.building_count*1) {
            ...
            'div.planetMapTileActionContainer',
            this,
            true
          ); //"button.planetMapTileActionButton"
        } else {
          this.MapVisible(true); //needs to be visible before we set sizing and  map
      ...
      NotIsolationist: function (building) {
        if (EmpireRPCStore.is_isolationist) {
          if (
            ...
            !confirm(
              'If you build an Espionage Ministry you will no longer be considered an Isolationist and you will be open to attack.  Are you sure you want to do this?'
            )
          ) {
            return true;
          } else if (
            building.url == '/munitionslab' &&
            !confirm(
              'If you build a Munitions Lab you will no longer be considered an Isolationist and you will be open to attack.  Are you sure you want to do this?'
            )
          ) {
            return true;
          }
      ...
      DetailsViewLegacy: function (tile) {
        ...
        this.currentViewConnection = this.ViewData(
          tile.data.id,
          tile.data.url,
          {
      ...
      DetailsProcess: function (oResults, url, x, y) {
        ...
        if (panel.isVisible() && currBuildingId != oResults.building.id) {
          ...
          if (building.pending_build) {
            panel.timeLeftLi.innerHTML =
              '<label>Build Time Remaining:</label> ' +
              Lib.formatTime(building.pending_build.seconds_remaining);
  ...
  YAHOO.register('mapPlanet', YAHOO.lacuna.MapPlanet, {
    version: '1',
    build: '0',
  });
}
