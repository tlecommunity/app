...
if (typeof YAHOO.lacuna.MapStar == 'undefined' || !YAHOO.lacuna.MapStar) {
  (function () {
    ...
    MapStar.prototype = {
      _buildDetailsPanel: _.once(function () {
        ...
        this.starDetails.hideEvent.subscribe(
          function () {
      ...
      _buildPlanetDetailsPanel: _.once(function () {
        ...
        panel.innerHTML = [
          ...
          this._excavLabel,
          '</em></a></li>',
          '            <li><a href="#planetDetailRename"><em>',
          this._renameLabel,
          '</em></a></li>',
          '            <li><a href="#planetDetailSendSpies"><em>',
          this._sendSpiesLabel,
          '</em></a></li>',
          '            <li><a href="#planetDetailFetchSpies"><em>',
          this._fetchSpiesLabel,
          '</em></a></li>',
          '        </ul>',
          '        <div class="yui-content">',
        ...
        this.planetDetails.renderEvent.subscribe(
          function () {
            ...
            var getShips = function (e) {
              if (e.newValue) {
                this.GetShips(this.planetDetails, {
            ...
            for (var tabId in spyTabs) {
              if (spyTabs.hasOwnProperty(tabId)) {
                ...
                tabEl.elShipMessageCount = Sel.query(
                  '.planetDetailSpyShipMessage span.count',
                  tabEl,
                  true
                );
                ...
                Event.delegate(
                  ...
                  'click',
                  this.MoveSpyShip,
                  '.planetDetailSelectSpyShip ul button',
                  this,
                  true
                );
      ...
      _buildFindPanel: _.once(function () {
        ...
        panel.id = panelId;
        panel.innerHTML = [
          '<div class="hd">Find</div>',
          '<div class="bd">',
          '    <label for="',
          panelId,
          'Find">By Star Name:</label><input type="text" id="',
          panelId,
          'Find" />',
          '    <hr />',
          '    <div>',
          '        <label for="',
          panelId,
          'X">X:</label><input type="text" id="',
          panelId,
          'X" size="4" />',
          '        <label for="',
          panelId,
          'Y">Y:</label><input type="text" id="',
          panelId,
          'Y" size="4" />',
        ...
        this.starFind.showEvent.subscribe(
          function () {
        ...
        if (ships.length > 0) {
          ...
          for (var i = 0; i < ships.length; i++) {
            ...
            nLi.innerHTML = [
              ...
              '    <div class="yui-u first" style="width:20%;background:transparent url(',
              Lib.AssetUrl,
              ...
              '] ',
              ship.name,
              ' - Arrives in: <span class="shipArrives">',
              Lib.formatTime(sec),
              '</span></div>',
              ...
              ship.from ? ship.from.name : 'Unknown',
              '</span></span>',
              ...
              '            <span>Speed:<span>',
              ship.fleet_speed > 0 && ship.fleet_speed < ship.speed ? ship.fleet_speed : ship.speed,
              '</span></span>,',
              '            <span>Hold Size:<span>',
              ship.hold_size,
              '</span></span>',
        ...
        if (ships.length === 0) {
          ...
          for (var i = 0; i < ships.length; i++) {
            ...
            nLi.innerHTML = [
              ...
              Lib.formatTime(ship.estimated_travel_time),
              '</span></span>',
              '        </div>',
              '        <div><label style="font-weight:bold;">Attributes:</label>',
              '            <span>Speed:<span>',
            ...
            if (ship.task == 'Docked') {
              Event.on(
                Sel.query('button', nLi, true),
                'click',
                this.ShipSend,
                { Self: this, Ship: ship, Line: nLi },
                true
              );
            }
          ...
          Game.Services.Buildings.SpacePort.send_ship(
            ...
            },
            {
              success: function (o) {
                ...
                this.Line.innerHTML =
                  'Successfully sent ' + this.Ship.type_human + ' to ' + targetName + '.';
              },
              scope: this,
            }
          );
      ...
      PopulateFleetSendTab: function (panel) {
        var ships = this.currentShips.available,
          ...
          btn = Dom.get(
            panel.isStarPanel ? 'starDetailSendFleetSubmit' : 'planetDetailSendFleetSubmit'
          ),
          detailsParent = details.parentNode,
          li = document.createElement('li');
      ...
      FleetSend: function (e) {
        ...
        var speed = parseInt(
          Dom.get(panel.isStarPanel ? 'starDetailSetSpeed' : 'planetDetailSetSpeed').value,
          10
        );
      ...
      PopulateShipsUnavailTab: function (panel) {
        ...
        if (ships.length === 0) {
          ...
          for (var i = 0; i < ships.length; i++) {
            ...
            nLi.Ship = ship;
            nLi.innerHTML = [
              '<div class="yui-gd" style="margin-bottom:5px;">',
              '    <div class="yui-u first" style="width:20%;background:transparent url(',
              Lib.AssetUrl,
              'star_system/field.png) no-repeat center;text-align:center;">',
              '        <img src="',
      ...
      ShowStar: function (tile, keepOpen) {
        ...
        Dom.get('starDetailsImg').innerHTML = [
          '<img src="',
          Lib.AssetUrl,
          'star_map/',
          data.color,
        ...
        Dom.get('starDetailsInfo').innerHTML = [
          ...
          data.zone,
          '</li>',
          '    <li><label>Star ID: </label>',
          data.id,
          '</li>',
          '    <li><label>Net Influence: </label>',
          data.influence,
          '</li>',
          data.station
            ? '    <li><label>Station: </label>' +
              data.station.name +
              ' (' +
              data.station.x +
              ' : ' +
              data.station.y +
              ')</li><li><label>Station ID: </label>' +
              data.station.id +
              '</li><li><label>Alliance: </label>' +
              data.station.alliance.name +
              '</li>'
            : '',
          '</ul>',
        ].join('');
      ...
      ShowPlanet: function (tile, keepOpen) {
        ...
        Game.Services.Body.get_body_status(data, {
          success: function (o) {
            ...
            Dom.get('planetDetailsImg').innerHTML = [
              '<img src="',
              Lib.AssetUrl,
              'star_system/',
              body.image,
            ...
            Dom.get('planetDetailsInfo').innerHTML = [
              '<ul>',
              '    <li id="planetDetailsName">',
              body.name,
              '</li>',
              '    <li><label>Type: </label>',
              body.type,
              '</li>',
              '    <li><label>Empire: </label><span id="planetDetailsEmpire">',
              empire.name,
              '</span></li>',
              empire.id
      ...
      MoveSpies: function (e, tab) {
        ...
        if (spies.length > tab.maxSpies) {
          alert(
            "You don't have any ships large enough to " + verb + ' ' + spies.length + ' spies.'
          );
          return;
        }
  ...
  YAHOO.register('mapStar', YAHOO.lacuna.MapStar, {
    version: '1',
    build: '0',
  });
}
