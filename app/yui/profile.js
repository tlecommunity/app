...
if (typeof YAHOO.lacuna.Profile == 'undefined' || !YAHOO.lacuna.Profile) {
  (function () {
    ...
    Profile.prototype = {
      build: _.once(function () {
        ...
        this.Dialog = new YAHOO.widget.Dialog(this.id, {
          ...
          buttons: [
            {
              text: 'Update',
              handler: { fn: this.handleUpdate, scope: this },
              isDefault: true,
            },
          ],
          visible: false,
        ...
        this.Dialog.hideEvent.subscribe(() => OptionsWindowStore.hide());
        this.Dialog.render();
          ...
          buttons: [
            {
              ...
              handler: { fn: this.redefineSpecies, scope: this },
              isDefault: true,
            },
            {
              text: 'Cancel',
        ...
        this.SpeciesDialog.renderEvent.subscribe(
          function () {
      ...
      handleUpdate: function () {
        ...
        if (
          Game.GetCookieSettings('disableDialogAnim', '0') != (this.stopAnim.checked ? '1' : '0')
        ) {
          var newEffect;
        ...
        if (Game.GetCookieSettings('showLevels', '0') != (this.showLevels.checked ? '1' : '0')) {
          ...
          for (var n = 0; n < levels.length; n++) {
            Dom.setStyle(
              levels[n].parentNode,
              'visibility',
              this.showLevels.checked ? 'visible' : ''
            );
          }
        }
      ...
      populateProfile: function (results) {
        ...
        for (var id in p.medals) {
          if (p.medals.hasOwnProperty(id)) {
            ...
            nLi.MedalId = id;
            nLi.innerHTML = [
              '    <div class="medalPublic"><input type="checkbox"',
              medal['public'] == '1' ? ' checked' : '',
              ' /></div>',
              ...
              medal.name,
              ' on ',
              Lib.formatServerDate(medal.date),
              '" />',
              '    </div>',
            ].join('');
      ...
      populateSpecies: function () {
        ...
        nLi = li.cloneNode(false);
        nLi.innerHTML = ['<label>Description</label>', '<span>', stat.description, '</span>'].join(
          ''
        );
        frag.appendChild(nLi);
        ...
        nLi.innerHTML = [
          ...
          '<span>',
          stat.management_affinity,
          '</span>',
        ].join('');
        frag.appendChild(nLi);

        nLi = li.cloneNode(false);
        nLi.innerHTML = ['<label>Farming</label>', '<span>', stat.farming_affinity, '</span>'].join(
          ''
        );
        frag.appendChild(nLi);

        nLi = li.cloneNode(false);
        nLi.innerHTML = ['<label>Mining</label>', '<span>', stat.mining_affinity, '</span>'].join(
          ''
        );
        frag.appendChild(nLi);

        nLi = li.cloneNode(false);
        nLi.innerHTML = ['<label>Science</label>', '<span>', stat.science_affinity, '</span>'].join(
          ''
        );
        frag.appendChild(nLi);
        ...
        nLi.innerHTML = [
          ...
          '<span>',
          stat.political_affinity,
          '</span>',
        ].join('');
        frag.appendChild(nLi);

        nLi = li.cloneNode(false);
        nLi.innerHTML = ['<label>Trade</label>', '<span>', stat.trade_affinity, '</span>'].join('');
        frag.appendChild(nLi);

        nLi = li.cloneNode(false);
        nLi.innerHTML = ['<label>Growth</label>', '<span>', stat.growth_affinity, '</span>'].join(
          ''
        );
        frag.appendChild(nLi);
        ...
        redefineButton.innerHTML = [
          ...
          ' <img src="',
          Lib.AssetUrl,
          'ui/s/essentia.png" class="smallEssentia smallImg" /> Redefine Species',
        ].join('');
      ...
      showSpeciesRedefine: function (e) {
        ...
        if (this.speciesRedefineLimits.can) {
          ...
          this.SpeciesDialog.getButtons()[0].innerHTML = [
            ...
            ' <img src="',
            Lib.AssetUrl,
            'ui/s/essentia.png" class="smallEssentia smallImg" /> Update',
          ].join('');
  ...
  YAHOO.register('profile', YAHOO.lacuna.Profile, {
    version: '1',
    build: '0',
  });
}
